...
<CodeBlock code="nslookup {domain} {dns}" queryParams={[{ name: 'domain', value: "app.example.com" }, { name: 'dns', value: "10.0.0.123" }]} />

Validate the commmand works without the IP.

<CodeBlock code="nslookup {domain}" queryParams={[{ name: 'domain', value: "app.example.com" }]} />
